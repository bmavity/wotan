  ...
  server.use(function(req, res, next) {
    ...
    if(canHandleRequest(httpRequest)) {
      transport.handleResourceRequest('http', httpRequest, function(resourceRequest, executedHandler) {
        var ehf = ve.executeHandlerFn(resourceRequest, executedHandler);
        if(resourceRequest.query) {
          var resource = wotan.getResource(resourceRequest.query).resource;
          console.log(wotan.getView(resourceRequest.query));
          ve.renderView(resource, ehf, renderHtml.bind({}, res));
        }
