    ...
    if(canHandleRequest(httpRequest)) {
      transport.handleResourceRequest('http', httpRequest, function(resourceRequest, executedHandler) {
        if(resourceRequest.query) {
          ve.renderView(res, wotan.getResource(resourceRequest.query).resource, ve.executeHandlerFn(resourceRequest, executedHandler));
        }
        if(resourceRequest.command) {
          renderJson(res, ve.executeHandlerFn(resourceRequest, executedHandler));
        }
      });
    } else {
      next();
