...
*/
        action = resource[actionName] = resource[actionName] || { name: actionName },
    ...
    if(query) {
      addRoute(resourceName, action, query);
      action.model = query;
    }
    if(command) {
      addRoute(resourceName, action, command);
      action.command = command;
    }
  };
};

var addRoute = function(resourceName, action, handlerFn) {
