...
var executeCommand = function(command, params) {
  ...
  return {
    actions: resources['sessions'].state.getActions(params.obj.id),
    model: model
  };
};
...
var getResource = function(path) {
  ...
  if(routeMatches.length) {
    ...
    parsedRoute = matchingAction.routeRegEx.exec(path);
    matchingAction.routeParams.forEach(function(paramName) {
